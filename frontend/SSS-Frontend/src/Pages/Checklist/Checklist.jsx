...
<div className="checklist-top-actions">
              <div className="checklist-searchable-dropdown">
                <input
                  type="text"
                  className="checklist-sheet-select"
                  value={sheetName}
                  placeholder="Select or type checklist"
                  onFocus={() => { setSheetSearch(""); setSheetDropdownOpen(true); }}
                  onChange={(e) => {
                ...
                {sheetDropdownOpen && (
                  <div className="checklist-dropdown-menu">
                    {sheetNames
                      .filter((s) =>
                      ...
                      .map((s) => (
                        <div
                          key={s}
                          className="checklist-dropdown-option"
                          onMouseDown={() => {
                            setSheetName(s);
                            setSelectedSection("");
              ...
              {["Soft","Technical"].includes(sheetName) && (
                <select
                  value={selectedSection}
                  onChange={(e) => setSelectedSection(e.target.value)}
                >
                  <option value="">-- Select Section --</option>
                  {[...new Set(checklist.map((item) => item.sectionName).filter(Boolean))].map((section) => (
            ...
            <div className="checklist-sheet">
              ...
              <div className="excel-header">
                <div className="excel-logo">
                  <img className="sss-logo" src={sssLogo} alt="SSS FMS Logo" />
                ...
                <table className="checklist-table excel-style-table">
                  ...
                  <tbody>
                    {rowList.map((row, index) => {
                      ...
                      const sectionValue = isMaster ? (entry.sectionName ?? row.item.sectionName ?? "") : (entry.sectionName || (row.item && row.item.sectionName) || "");
                      const taskValue = isMaster ? (entry.taskName ?? row.item.checkPoint ?? "") : (entry.taskName || (row.item && row.item.checkPoint) || "");
                        ...
                        <tr key={row.key} className={isBlankRow ? "empty-row" : ""}>
                          ...
                          <td className="bg-section">
                            <input
                              ...
                              value={sectionValue === "-" ? "" : sectionValue}
                              onChange={(e) =>
                                onCellEdit(row.uid, "sectionName", e.target.value)
                              }
                            />

                            <datalist id="section-options">
                              {[...new Set(TASK_OPTIONS.map((group) => group.section))]
                                .filter(Boolean)
                                .map((section) => (
                                  <option key={section} value={section} />
                                ))}
        ...
        >
          {emp.name?.trim()
            ...
            : emp.employeeId}
                          ...
                          <td>
                            <input
                              className="cell-input updated-by-cell"
                              type="text"
                              list={`updated-by-options-${row.uid}`}
                              placeholder="Select or type name"
                              value={entry.updatedBy || ""}
                              onChange={(e) =>
                                onCellEdit(row.uid, "updatedBy", e.target.value)
                              }
                            />

                            <datalist id={`updated-by-options-${row.uid}`}>
                              {employees
                                .filter(
                                  (emp) =>
                                    emp &&
                                ...
                                .map((emp) => (
                                  <option
                                    key={emp.id}
                                    value={emp.name?.trim() || emp.employeeId?.trim()}
                                  />
                                ))}
                            </datalist>
                          </td>
